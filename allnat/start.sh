...
# This script starts the virtual machines and the Kubernetes cluster.
...
if [[ "${#VMs[@]}" -gt 0 ]]; then
  echo 'Starting VMs in the isolated VPC.'
  gcloud compute instances start "$VMs" \
    --project="$TF_VAR_isolated_vpc_pid"  \
    --zone="$TF_VAR_zone"
else
    echo 'There are no VMs in the isolated VPC to start.'
fi
...
if [[ "${#VMs[@]}" -gt 0 ]]; then
  echo 'Starting VMs in the shared VPC.'
  gcloud compute instances start "$VMs" \
    --project="$TF_VAR_shared_vpc_pid" \
    --zone="$TF_VAR_zone"
else
...
echo 'Resizing cluster to 3 VMs.'
